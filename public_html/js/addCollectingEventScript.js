...
function buildLocalityID(){
  ...
  req.open('GET','https://johnhash.me/flies/localityDropdown', true);
...
document.getElementById('enterCollectingEvent').addEventListener('click', function(event){
  ...
  let payload = '?locality_id='+locIdInput+'&method='+methodInput+'&start_date='+startDateInput+'&end_date='+endDateInput+'&appID='+userId;
  
  //open the request to /addLocality
  req.open('GET','https://johnhash.me/flies/addCollectingEvent'+payload, true);
  req.setRequestHeader('Content-Type', 'application/json');
